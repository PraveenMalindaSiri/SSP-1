    ...
    <div class="my_form_div">
        ...
        <form action="/cb008920/public/register" method="post">
            <div>
                ...
                <input class="my_input" type="text" id="username" name="username" required value="<?= htmlspecialchars($old['username'] ?? '') ?>">
                <p class="text-red"><?= $errors['username'] ?? '' ?></p>
            </div>
            <div>
                <label for="password">Password</label><br>
                <input class="my_input" type="password" id="password" name="password" required value="<?= htmlspecialchars($old['password'] ?? '') ?>">
                <p class="text-red"><?= $errors['password'] ?? ''  ?></p>
            </div>
            <div>
                <label for="fullname">Fullname</label><br>
                <input class="my_input" type="text" id="fullname" name="fullname" required value="<?= htmlspecialchars($old['fullname'] ?? '') ?>">
                <p class="text-red"><?= $errors['fullname'] ?? ''  ?></p>
            </div>
            <div>
                <label for="address">Address</label><br>
                <input class="my_input" type="text" id="address" name="address" required value="<?= htmlspecialchars($old['address'] ?? '') ?>">
                <p class="text-red"><?= $errors['address'] ?? ''  ?></p>
            </div>
            <div>
                <label for="email">Email</label><br>
                <input class="my_input" type="email" id="email" name="email" required value="<?= htmlspecialchars($old['email'] ?? '') ?>">
                <p class="text-red"><?= $errors['email'] ?? ''  ?></p>
            </div>
            <div>
                <label for="dob">Date of Birth (DoB)</label><br>
                <input class="my_input" type="date" id="dob" name="dob" required value="<?= htmlspecialchars($old['dob'] ?? '') ?>">
                <p class="text-red"><?= $errors['email'] ?? ''  ?></p>
            </div>
            <div class="w-[50%]">
